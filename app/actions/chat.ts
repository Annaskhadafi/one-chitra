...
export async function searchDocumentsForMention(query: string) {
    ...
    const formatPoDetails = (customerName: string | null, totalQty: number, totalValue: string) => {
        const normalizedQty = Number(totalQty ?? 0)
        const normalizedValue = Number(totalValue ?? 0)
        return `${customerName || "Customer -"} • Qty ${normalizedQty.toLocaleString("id-ID")} • Value Rp ${normalizedValue.toLocaleString("id-ID")}`
    }
    ...
    for (const s of poShortcutResults ?? []) {
        soMap.set(String(s.id), {
            ...
            sublabel: s.number || s.subject || `Sales Order #${s.id}`,
            details: formatPoDetails(s.customerName, Number(s.totalQty ?? 0), s.totalValue ?? "0"),
