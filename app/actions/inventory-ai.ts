...
// Default AI settings (fallback if database settings are not available)
...
/**
 * Get AI settings from database with fallback to defaults
 * Requirements: 9.9
 */
export async function getAISettings() {
    try {
        ...
        return {
            model: settingsMap.get('ai_model') || DEFAULT_AI_SETTINGS.model,
            temperature: parseFloat(settingsMap.get('ai_temperature') || String(DEFAULT_AI_SETTINGS.temperature)),
            maxTokens: parseInt(settingsMap.get('ai_max_tokens') || String(DEFAULT_AI_SETTINGS.maxTokens)),
            cacheDuration: parseInt(settingsMap.get('ai_cache_duration') || String(DEFAULT_AI_SETTINGS.cacheDuration)),
            thinkingMode: settingsMap.get('ai_thinking_mode') === 'true'
        };
    } catch (error) {
        console.error("Failed to fetch AI settings, using defaults:", error);
...
Format Response Anda HARUS valid JSON saja, tanpa markdown text lain:
{
  ...
  "rationale": "Penjelasan singkat mengenai cara menentukan Safety Stock tanpa tanda bintang (**)."
        ...
        console.log("[AI] Calling Groq API with model:", aiConfig.model, "for product:", productCode);
...
export async function getSalesHistory(materialNo: string) {
    try {
        ...
        const salesData = await db.select({
            ...
            .where(
                and(
                    ilike(salesRevenueSap.materialNo, `%${materialNo}%`),
                    sql`${salesRevenueSap.billingDate} >= ${sixMonthsAgo}`
                )
            )
            .groupBy(sql`to_char(${salesRevenueSap.billingDate}, 'YYYY-MM')`)
            .orderBy(sql`to_char(${salesRevenueSap.billingDate}, 'YYYY-MM')`);
...
/**
 * Test AI settings with sample data
 * Requirements: 9.10
 */
export async function testAISettings(testSettings: {
    model: string;
    temperature: number;
    maxTokens: number;
}) {
 ...
 * Requirements: 9.12
 */
export async function resetAISettings(updatedBy: string) {
    try {
        await getAuthenticatedSession("inventory", "edit");
