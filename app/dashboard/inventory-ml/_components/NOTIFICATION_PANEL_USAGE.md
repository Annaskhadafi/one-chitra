...
```tsx
import { NotificationPanel } from "@/app/dashboard/inventory-ml/_components/notification-panel"
...
export default function DashboardPage() {
  ...
  const handleNavigateToDetail = (productCode: string) => {
    ...
    router.push(`/dashboard/inventory-ml/detail?product=${productCode}`)
  }
...
The component uses the following server actions from `app/actions/inventory-ml.ts`:
