...
The filter state management system provides a centralized way to manage filter preferences across the ML Inventory Forecast feature with automatic localStorage persistence.
...
export default function InventoryMLLayout({ children }) {
  return (
    <FilterProvider>
      {children}
    </FilterProvider>
...
- Storage key: `"inventory-ml-filters"`
...
Run tests:
```bash
npm test -- app/dashboard/inventory-ml/_components/__tests__/filter-context.test.tsx
```
