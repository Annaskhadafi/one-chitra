...
// ── Customer Selector Component (Optimized Lazy Loading) ─────────────────────
interface CustomerSelectorProps {
    customers: CoverLetterCustomer[];
    selectedCustomer: CoverLetterCustomer | null;
    onSelect: (customer: CoverLetterCustomer) => void;
}

function CustomerSelector({ customers, selectedCustomer, onSelect }: CustomerSelectorProps) {
        ...
        <Popover open={open} onOpenChange={setOpen}>
            <PopoverTrigger asChild>
                <Button variant="outline" role="combobox" aria-expanded={open} className="w-full justify-between font-normal">
                    {selectedCustomer ? <span className="truncate font-medium">{selectedCustomer.name}</span> : <span className="text-muted-foreground">Pilih customer...</span>}
                ...
                <PopoverContent className="w-[400px] p-0" align="start">
                    <Command shouldFilter={false}>
                        ...
                        <CommandList>
                            ...
                            <CommandGroup heading={search.trim() ? "Hasil Pencarian" : "Daftar Customer (50 Teratas)"}>
                                {displayedCustomers.map(c => (
                                    <CommandItem
                                        key={c.id}
                                        value={c.customerCode + " " + c.name}
                                        onSelect={() => {
                                            onSelect(c);
                                            setOpen(false);
                                            setSearch("");
...
export function CoverLetterClient({ customers, savedLetters: initialSavedLetters, initialSigners }: Props) {
    ...
    useEffect(() => {
        ...
        generateNextRefNumber(sendLocation, letterDate ? new Date(letterDate) : undefined)
            ...
            .catch(err => console.error("Failed to generate ref:", err))
    ...
    return (
        <div className="space-y-6" suppressHydrationWarning>
            ...
            <Card>
                <CardHeader className="pb-3">
                    <div className="flex items-center justify-between">
                        ...
                        <Button type="button" size="sm" onClick={() => handleNewLetter()} className="gap-2 cursor-pointer relative z-10">
            ...
            <Dialog open={showForm} onOpenChange={(open) => {
                ...
                <DialogContent className="max-w-5xl max-h-[90vh] flex flex-col p-0 overflow-hidden sm:max-w-5xl">
                    <DialogHeader className="px-6 py-4 border-b shrink-0">
                        <DialogTitle className="text-lg font-bold flex items-center gap-2">
                            <FileText className="h-5 w-5 text-primary" />
                            {editingId ? "Edit Cover Letter" : "Buat Cover Letter Baru"}
                        </DialogTitle>
                        <DialogDescription>
                            {editingId ? "Ubah detail cover letter dan daftar invoice yang terpilih." : "Isi detail customer, pilih invoice, dan tentukan penandatangan untuk membuat cover letter baru."}
                        </DialogDescription>
                    </DialogHeader>
                    ...
                    <DialogFooter className="px-6 py-4 border-t bg-muted/30 flex items-center justify-between sm:justify-between gap-3 shrink-0">
                        <Button variant="ghost" onClick={() => { setShowForm(false); resetForm(); }}>
                            Batal
                        </Button>
                        <div className="flex items-center gap-2">
                            <Button variant="outline" onClick={handlePreview} disabled={!selectedCustomer || previewItems.length === 0} className="gap-2">
                                <Eye className="h-4 w-4" /> Preview
                            </Button>
                            <Button onClick={handleSave} disabled={isSaving || !selectedCustomer || previewItems.length === 0} className="gap-2">
