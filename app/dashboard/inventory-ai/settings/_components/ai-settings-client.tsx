...
import { updateAISettings, testAISettings, resetAISettings } from "@/app/actions/inventory-ai"
...
export function AISettingsClient({ initialSettings, userId }: AISettingsClientProps) {
    ...
    const handleSave = async () => {
        ...
        try {
            const result = await updateAISettings(settings, userId)
    ...
    const handleTest = async () => {
        ...
        try {
            ...
            if (result.success) {
                setTestResult({
                    success: true,
                    message: "Test successful! AI responded correctly with the current parameters."
                })
                toast.success("AI parameters are working correctly")
            } else {
                setTestResult({
                    success: false,
            ...
            toast.error("Failed to test AI settings")
        } finally {
            setIsTesting(false)
        }
    }
    ...
    const handleReset = async () => {
        ...
        try {
            const result = await resetAISettings(userId)
            ...
            if (result.success) {
                ...
                toast.success("AI settings have been reset to defaults")
            } else {
        ...
        <div className="space-y-6">
            ...
            <Card>
                <CardHeader>
                    ...
                    <CardDescription>
                        Configure the AI model and parameters used for inventory predictions
                    </CardDescription>
                </CardHeader>
                <CardContent className="space-y-6">
                    ...
                    <div className="space-y-2">
                        ...
                        />
                        <p className="text-sm text-muted-foreground">
                            Maximum length of AI response (1000-8192). Higher values allow longer explanations.
                        </p>
                    </div>
                    ...
                    <div className="flex items-center justify-between space-x-2">
                        <div className="space-y-0.5">
                            ...
                            <p className="text-sm text-muted-foreground">
                                Enable AI to show reasoning process (may increase response time)
                            </p>
                        </div>
                        <Switch
                            id="thinkingMode"
