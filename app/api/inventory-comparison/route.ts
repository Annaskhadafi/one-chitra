import { NextResponse } from "next/server"
import { sql } from "drizzle-orm"
import { db } from "@/db"
import { normalizeSloc, normalizedSlocSql } from "@/lib/sloc"

export const dynamic = "force-dynamic"
export const maxDuration = 60

/**
 * Optimized API endpoint for inventory comparison
 * Returns pre-aggregated comparison data instead of raw data
 */
export async function GET() {
    try {
        const normalizedWarehouseSloc = normalizedSlocSql(sql`w.sloc`)
        const normalizedSapSloc = normalizedSlocSql(sql`stor_loc`)

        // Get comparison data with aggregation in SQL for better performance
        const result = await db.execute(sql`
            WITH local_stocks_raw AS (
                SELECT 
                    COALESCE(UPPER(TRIM(p.material_number)), '') as material_number,
                    NULLIF(TRIM(p.material_description), '') as material_description,
                    NULLIF(TRIM(p.plant), '') as plant,
                    NULLIF(TRIM(p.category), '') as category,
                    ${normalizedWarehouseSloc} as sloc,
                    NULLIF(TRIM(w.description), '') as sloc_desc,
                    NULLIF(TRIM(w.type), '') as warehouse_type,
                    COALESCE(sl.total_stock, 0)::numeric as local_stock
                FROM stock_levels sl
                JOIN products p ON sl.product_id = p.id
                JOIN warehouses w ON sl.warehouse_id = w.id
                WHERE w.type NOT ILIKE '%repair%'
            ),
            local_stocks AS (
                SELECT
                    material_number,
                    MAX(material_description) as material_description,
                    MAX(plant) as plant,
                    MAX(category) as category,
                    sloc,
                    MAX(sloc_desc) as sloc_desc,
                    MAX(warehouse_type) as warehouse_type,
                    SUM(local_stock) as local_stock
                FROM local_stocks_raw
                WHERE material_number <> ''
                  AND sloc <> ''
                GROUP BY material_number, sloc
            ),
            sap_stocks_raw AS (
                SELECT 
                    COALESCE(UPPER(TRIM(material_no)), '') as material_number,
                    NULLIF(TRIM(material_desc), '') as material_description,
                    NULLIF(TRIM(plant_code), '') as plant,
                    ${normalizedSapSloc} as sloc,
                    NULLIF(TRIM(stor_loc_desc), '') as sloc_desc,
                    COALESCE(total_stock::numeric, 0) as sap_stock
                FROM zmc9_stock_sap
                WHERE plant_code != '2002'
            ),
            sap_stocks AS (
                SELECT
                    material_number,
                    MAX(material_description) as material_description,
                    MAX(plant) as plant,
                    sloc,
                    MAX(sloc_desc) as sloc_desc,
                    SUM(sap_stock) as sap_stock
                FROM sap_stocks_raw
                WHERE material_number <> ''
                  AND sloc <> ''
                GROUP BY material_number, sloc
            )
            SELECT 
                COALESCE(ls.material_number, ss.material_number) as material_number,
                COALESCE(ls.material_description, ss.material_description) as description,
                COALESCE(ls.sloc, ss.sloc) as sloc,
                COALESCE(ls.sloc_desc, ss.sloc_desc) as sloc_desc,
                COALESCE(ls.warehouse_type, '') as warehouse_type,
                COALESCE(ls.category, '') as category,
                COALESCE(ls.plant, ss.plant) as plant,
                COALESCE(ls.local_stock, 0) as local_stock,
                COALESCE(ss.sap_stock, 0) as sap_stock,
                (COALESCE(ls.local_stock, 0) - COALESCE(ss.sap_stock, 0)) as gap
            FROM local_stocks ls
            FULL OUTER JOIN sap_stocks ss 
                ON ls.material_number = ss.material_number
                AND ls.sloc = ss.sloc
            ORDER BY ABS(COALESCE(ls.local_stock, 0) - COALESCE(ss.sap_stock, 0)) DESC
        `)

        const rows = result.rows as Array<{
            material_number: string
            description: string
            sloc: string
            sloc_desc: string
            warehouse_type: string
            category: string
            plant: string
            local_stock: number
            sap_stock: number
            gap: number
        }>

        const comparisonData = rows.map((row) => ({
            materialNumber: row.material_number || "",
            description: row.description || "",
            sloc: normalizeSloc(row.sloc || ""),
            slocDesc: row.sloc_desc || "",
            warehouseType: row.warehouse_type || "",
            category: row.category || "",
            plant: row.plant || "",
            localStock: Number(row.local_stock) || 0,
            sapStock: Number(row.sap_stock) || 0,
            gap: Number(row.gap) || 0,
            status: Number(row.gap) === 0 ? "match" : Number(row.gap) > 0 ? "over" : "under"
        }))

        // Calculate stats
        const stats = {
            total: comparisonData.length,
            matched: comparisonData.filter(r => r.status === "match").length,
            over: comparisonData.filter(r => r.status === "over").length,
            under: comparisonData.filter(r => r.status === "under").length,
            totalAbsGap: comparisonData.reduce((sum, r) => sum + Math.abs(r.gap), 0),
            withGap: comparisonData.filter(r => r.gap !== 0).length,
        }

        return NextResponse.json({
            status: "OK",
            data: comparisonData,
            stats,
        })
    } catch (error) {
        console.error("Failed to fetch inventory comparison:", error)
        return NextResponse.json(
            {
                status: "ERROR",
                message: "Failed to fetch inventory comparison",
                error: error instanceof Error ? error.message : "Unknown error"
            },
            { status: 500 }
        )
    }
}
