...
interface EvhsMasterPriceModalProps {
    open: boolean
    onOpenChange: (open: boolean) => void
}

export function EvhsMasterPriceModal({ open, onOpenChange }: EvhsMasterPriceModalProps) {
    return (
            ...
            <DialogContent className="!left-1/2 !top-1/2 !h-[94vh] !max-h-none !w-[96vw] !max-w-[1720px] !-translate-x-1/2 !-translate-y-1/2 rounded-[28px] border border-slate-200 p-0 flex flex-col overflow-hidden shadow-[0_32px_120px_rgba(15,23,42,0.30)]">
                <DialogHeader className="shrink-0 border-b p-4">
                    ...
                    <DialogDescription className="sr-only">
                        Embedded master price CK reference in a fullscreen dialog.
                    </DialogDescription>
                </DialogHeader>

                <div className="flex-1 bg-muted/10">
                    <iframe
                        src="/evhs-master-price-modal"
                        className="h-full w-full border-0"
                        title="Master Price CK"
                    />
                </div>
            </DialogContent>
        </Dialog>
