  ...
  return (
    <Card className="h-full border-none bg-white shadow-sm">
      ...
      <CardContent>
        <div className={isSidebar ? "grid grid-cols-1 gap-2 sm:grid-cols-2 xl:grid-cols-1" : "grid grid-cols-1 gap-3 sm:grid-cols-2 xl:grid-cols-5"}>
          {QUICK_ACCESS_ITEMS.map((item) => (
            <Link
              ...
              href={item.href}
              className={isSidebar
                ? "group relative overflow-hidden rounded-2xl border border-slate-200 bg-gradient-to-r from-white via-slate-50/80 to-slate-100/80 px-3 py-2.5 transition-all duration-200 hover:-translate-y-0.5 hover:border-slate-300 hover:shadow-md focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-slate-300"
                : "group relative overflow-hidden rounded-2xl border border-slate-200 bg-gradient-to-br from-white via-slate-50/70 to-slate-100/80 p-4 transition-all duration-200 hover:-translate-y-0.5 hover:border-slate-300 hover:shadow-md focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-slate-300"}
            >
              {isSidebar ? (
                <div className="flex items-center justify-between gap-3">
                  <div className="flex min-w-0 items-center gap-3">
                    ...
                    <p className="truncate text-[13px] font-semibold leading-5 text-slate-900">{item.title}</p>
                  </div>
        ...
        {!isSidebar ? (
          <div className="mt-3 flex items-center gap-2 rounded-2xl border border-dashed border-slate-200 bg-slate-50 px-3 py-2 text-[11px] text-slate-500">
