...
export const SYSTEM_EMAIL_TEMPLATE_CODES = {
    ...
    inventoryProcurementRequest: "inventory_procurement_request",
    stockMinimumAlert: "stock_minimum_alert",
    deliveryScheduledReminder: "delivery_scheduled_reminder",
    salesOrderCreatedSalesPic: "sales_order_created_sales_pic",
    deliveryCreatedSalesPic: "delivery_created_sales_pic",
    ...
    chatUnreadReminder: "chat_unread_reminder",
...
export const SYSTEM_EMAIL_TEMPLATES: SystemEmailTemplateDefinition[] = [
    ...
    },
    {
        ...
        name: "Inventory Procurement Request",
        type: "notification",
        subject: "[Procurement] Permintaan PO {{urgencyLevel}} - {{productCode}} / {{warehouseLabel}}",
        variables: [
            "requestorName",
            "productCode",
            "productName",
        ...
        defaultActive: true,
        ...createStructuredTemplate({
            title: "Permintaan Pembuatan PO",
            subtitle: "Permintaan ini dibuat dari dashboard Inventory Control Intelligence untuk percepatan replenishment.",
            accentColor: "#be123c",
            buttonLabel: "Buka Inventory Dashboard",
            buttonHref: "{{actionUrl}}",
            rows: [
                ...
                { label: "Average Usage / Day", value: "{{avgDailyUsage}}" },
                { label: "Days to Stockout", value: "{{daysToStockout}}" },
                { label: "Recommended Restock Qty", value: "{{recommendedRestockQty}}" },
                { label: "Estimated Restock Value", value: "{{recommendedRestockValue}}" },
                { label: "Urgency", value: "{{urgencyLevel}}" },
            ],
            extraHtml: `
                <p style="margin:0 0 16px;font-size:14px;line-height:1.6;color:#334155;">
                    Mohon tim procurement segera menindaklanjuti kebutuhan pembelian ke vendor sesuai item di atas.
                    Penerima email ini dapat diatur dari menu pengelolaan template email sistem.
                </p>
            `,
            extraText: [
                "Mohon tim procurement segera menindaklanjuti kebutuhan pembelian ke vendor sesuai item di atas.",
    ...
    },
    {
        ...
        name: "Sales Order Created to Sales PIC",
        type: "notification",
        subject: `[${APP_NAME}] Sales Order dibuat - {{salesOrderNumber}}`,
        variables: [
            "salesPicName",
            "salesOrderNumber",
            "customerPo",
        ...
        ...createStructuredTemplate({
            ...
            subtitle: "Notifikasi untuk PIC Sales terkait Sales Order baru.",
            accentColor: "#1d4ed8",
            buttonLabel: "Buka Sales Order",
            buttonHref: "{{actionUrl}}",
            rows: [
                { label: "Sales PIC", value: "{{salesPicName}}" },
                { label: "No. Sales Order", value: "{{salesOrderNumber}}" },
                { label: "No. PO Customer", value: "{{customerPo}}" },
                { label: "Customer", value: "{{customerName}}" },
                { label: "Tanggal Sales", value: "{{salesDate}}" },
                { label: "Status", value: "{{statusLabel}}" },
                { label: "Warehouse", value: "{{warehouseName}}" },
                { label: "Dibuat Oleh", value: "{{createdByName}}" },
            ],
        }),
    },
    {
        code: SYSTEM_EMAIL_TEMPLATE_CODES.deliveryCreatedSalesPic,
        name: "Delivery Created to Sales PIC",
        type: "delivery_update",
        subject: `[${APP_NAME}] Delivery dibuat - {{deliveryNumber}}`,
        variables: [
            "salesPicName",
            "deliveryNumber",
            "salesOrderNumber",
        ...
        defaultActive: false,
        ...createStructuredTemplate({
            title: "Delivery Berhasil Dibuat",
            subtitle: "Notifikasi untuk PIC Sales terkait delivery baru.",
            accentColor: "#0369a1",
            buttonLabel: "Buka Delivery",
            buttonHref: "{{actionUrl}}",
            rows: [
                { label: "Sales PIC", value: "{{salesPicName}}" },
                { label: "No. Delivery", value: "{{deliveryNumber}}" },
                { label: "Sales Order", value: "{{salesOrderNumber}}" },
                { label: "No. PO Customer", value: "{{customerPo}}" },
                { label: "Customer", value: "{{customerName}}" },
                { label: "Tanggal Schedule", value: "{{scheduledDate}}" },
                { label: "Tipe Delivery", value: "{{deliveryTypeLabel}}" },
                { label: "Status Delivery", value: "{{statusLabel}}" },
                { label: "Warehouse Asal", value: "{{warehouseName}}" },
                { label: "Alamat Kirim", value: "{{shippingAddress}}" },
                { label: "Dibuat Oleh", value: "{{createdByName}}" },
            ],
    ...
    },
    {
        ...
        name: "Chat Unread Reminder",
        type: "notification",
        subject: "[Chat] {{unreadCount}} pesan belum dibaca di {{roomName}}",
        variables: ["recipientName", "roomName", "unreadCount", "latestSenderName", "latestUnreadAt", "latestMessagePreview", "actionUrl", "appName"],
        recipientRoles: [],
        recipientUserIds: [],
        ccEmails: [],
        defaultActive: true,
        ...createStructuredTemplate({
            title: "Pesan Chat Belum Dibaca",
            subtitle: "Ada percakapan yang belum Anda buka lebih dari 1 jam.",
            accentColor: "#0f172a",
            buttonLabel: "Buka Dashboard",
            buttonHref: "{{actionUrl}}",
            rows: [
                { label: "Penerima", value: "{{recipientName}}" },
                { label: "Room", value: "{{roomName}}" },
                { label: "Jumlah Unread", value: "{{unreadCount}}" },
                { label: "Pesan Terakhir Dari", value: "{{latestSenderName}}" },
                { label: "Waktu Pesan Terakhir", value: "{{latestUnreadAt}}" },
                { label: "Preview", value: "{{latestMessagePreview}}" },
            ],
        }),
    },
    {
