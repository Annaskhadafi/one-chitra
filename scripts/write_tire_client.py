...
import {
    Search, RefreshCw, Download, Filter, ChevronDown, ChevronRight,
    TrendingUp, Users, DollarSign, Package, Calendar, Building2, Layers
} from "lucide-react"
...
function CustomerRow({ customer, allGroups }: { customer: TireHistoryCustomer; allGroups: string[] }) {
    ...
    return (
        <Collapsible open={open} onOpenChange={setOpen}>
            <CollapsibleTrigger asChild>
                <TableRow className="cursor-pointer hover:bg-muted/40 group">
                    ...
                    <TableCell>
                        <div className="flex flex-wrap gap-1">
                            {Object.entries(customer.matGroups)
                                                ...
                                                <table className="w-full text-xs">
                                                    <thead>
                                                        <tr className="border-b text-muted-foreground">
                                                            <th className="text-left py-1 pr-4 font-medium">Material Group</th>
                                                            <th className="text-left py-1 pr-4 font-medium">Deskripsi</th>
                                                        ...
                                                        {data.items
                                                            ...
                                                            .map((item, idx) => (
                                                                <tr key={idx} className="border-b border-muted/50 hover:bg-muted/30">
                                                                    <td className="py-1 pr-4 text-muted-foreground font-mono">{item.matGrpDesc}</td>
                                                                    <td className="py-1 pr-4 font-medium">{item.materialDescription || "-"}</td>
                                                                    <td className="py-1 pr-4 text-right">{item.totalQty.toLocaleString()}</td>
                                                                    <td className="py-1 pr-4 text-right font-medium">{formatIDR(item.totalRevenue)}</td>
...
export function CustomerTireHistoryClient() {
    ...
    const topCustomersChart = useMemo(() => {
        ...
        return filteredCustomers.slice(0, 10).map(c => ({
            name: c.customerName.replace(/^PT\.?\s*/i, "").replace(/^CV\.?\s*/i, "").substring(0, 20),
            fullName: c.customerName,
            revenue: c.totalRevenue,
        }))
    }, [filteredCustomers])

    // Chart data: revenue by group
    const groupRevenueChart = useMemo(() => {
        if (!filteredCustomers.length) return []
        const map = new Map<string, number>()
        for (const c of filteredCustomers) {
            for (const [group, data] of Object.entries(c.matGroups)) {
                map.set(group, (map.get(group) || 0) + data.revenue)
            }
        }
    ...
    // Chart data: revenue by biz category
                            ...
                            <Table>
                                ...
                                <TableBody>
                                    {filteredCustomers.length === 0 ? (
                                        ...
                                        <CustomerRow key={customer.customerName} customer={customer} allGroups={data.matGrpGroupList} />
                                    ))}
                                </TableBody>
                            </Table>
                        </div>
